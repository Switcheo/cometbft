...
func (env *Environment) TxSearch(
	...
	// Validate order_by before doing any work. Sorting and pagination now
	// happen inside the indexer, so we only need to translate the option.
	...
	// Validate number of results per page and default the page to 1. These are
	// passed down so the indexer can sort a lightweight hash set and fetch only
	// the requested page, instead of loading every matching TxResult into memory.
